...
Course::Course(string id, string s, int c, Degree *d, Link_us_res **t)
    :Resource(id, s)
{
    degree=d;
    credits=c;
...
Course& Course::operator= (const Course &c){
    if (c.identification.c_str()==nullptr || c.status.c_str()==nullptr || c.degree==nullptr){
         std::cerr<<"Course:operator= : (Warning) some atribute of original object is invalid.\n";
    }else{
